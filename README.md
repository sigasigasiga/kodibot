...
> **Note:** These instructions assume kodibot runs on the same machine as your Kodi instance. If you run kodibot on a different machine, adjust the configuration (e.g. IP addresses, ports) accordingly.

### Obtaining Credentials

#### 1. Enable Remote Control in Kodi

Enabling remote control via HTTP is required for kodibot to communicate with Kodi. Follow these steps:
...
Installation instructions are organized by platform below. Currently, only Linux (via Podman) is documented.


> Running kodibot with Docker/Podman on other platforms (including Windows and macOS) should also be possible, but instructions haven't been written yet. If you've successfully run kodibot on another platform, please consider contributing your steps to this section—pull requests are welcome!

#### Linux (Podman)

##### Prerequisites

- Podman installed ([installation guide](https://podman.io/docs/installation))
...
- **Error: `creating container storage: not enough unused IDs in user namespace`**
  ...
  Then, run `podman system migrate` to apply the changes.

<!--
  Add new platform sections here, following the same structure, e.g.:
...
When kodibot receives a video file from Telegram, it creates a unique HTTP endpoint that serves as a streaming bridge. Immediately after that, the bot sends a command to Kodi to play the video hosted on that endpoint. Accessing this endpoint triggers the download of the requested video chunks directly from Telegram. As each chunk is downloaded, it is immediately forwarded to the client (i.e. your Kodi instance), enabling seamless streaming of large files without requiring the entire video to be downloaded to the bot's machine first.
...
If you like this project and want to support it, you can star it on GitHub.
